...
You can preprocess the dataset with `sevenn_graph_build` to obtain `./sevenn_data/graph.pt` files. They can be used for training (`sevenn`) or
inference (`sevenn_inference`), skipping graph build stage. `./sevenn_data/graph.yaml` contains statistics and meta information for the dataset.
Those files must be located under the `sevenn_data`. If you move the dataset, move the whole `sevenn_data` directory, without changing the contents.
