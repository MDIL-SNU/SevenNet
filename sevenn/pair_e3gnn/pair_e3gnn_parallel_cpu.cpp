...
#include "pair_e3gnn_parallel_cpu.h"
#include <cassert>
...
PairE3GNNParallelCPU::PairE3GNNParallelCPU(LAMMPS *lmp) : Pair(lmp) {
  // constructor
  ...
  if (lmp->screen) {
    fprintf(lmp->screen, "PairE3GNNParallelCPU using device : %s\n",
            device_name.c_str());
    fprintf(lmp->screen, "PairE3GNNParallelCPU cuda-aware mpi: %s\n",
            use_cuda_mpi ? "True" : "False");
    fprintf(lmp->screen, "Aten get num threads: %d\n", at::get_num_threads());
  }
  if (lmp->logfile) {
    fprintf(lmp->logfile, "PairE3GNNParallelCPU using device : %s\n",
            device_name.c_str());
    fprintf(lmp->logfile, "PairE3GNNParallelCPU cuda-aware mpi: %s\n",
            use_cuda_mpi ? "True" : "False");
    fprintf(lmp->screen, "Aten get num threads: %d\n", at::get_num_threads());
  }
}

PairE3GNNParallelCPU::~PairE3GNNParallelCPU() {
  if (allocated) {
...
int PairE3GNNParallelCPU::get_x_dim() { return x_dim; }

bool PairE3GNNParallelCPU::use_cuda_mpi_() { return use_cuda_mpi; }

bool PairE3GNNParallelCPU::is_comm_preprocess_done() {
  return comm_preprocess_done;
}

void PairE3GNNParallelCPU::compute(int eflag, int vflag) {
  /*
     Graph build on cpu
  */
  if (eflag || vflag)
...
void PairE3GNNParallelCPU::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairE3GNNParallelCPU::settings(int narg, char **arg) {
  if (narg != 0) {
    error->all(FLERR, "Illegal pair_style command");
  }
}

void PairE3GNNParallelCPU::coeff(int narg, char **arg) {
  if (allocated) {
...
void PairE3GNNParallelCPU::init_style() {
...
double PairE3GNNParallelCPU::init_one(int i, int j) { return cutoff; }

void PairE3GNNParallelCPU::notify_proc_ids(const int *sendproc, const int *recvproc) {
...
void PairE3GNNParallelCPU::comm_preprocess() {
...
void PairE3GNNParallelCPU::pack_forward_init(int n, int *list_send,
                                          int comm_phase) {
...
// called from comm_brick if comm_preprocess_done is false
void PairE3GNNParallelCPU::unpack_forward_init(int n, int first, int comm_phase) {
...
int PairE3GNNParallelCPU::pack_forward_comm_gnn(float *buf, int comm_phase) {
  std::vector<long> &idx_map = comm_index_pack_forward[comm_phase];
...
void PairE3GNNParallelCPU::unpack_forward_comm_gnn(float *buf, int comm_phase) {
  std::vector<long> &idx_map = comm_index_unpack_forward[comm_phase];
  const int n = static_cast<int>(idx_map.size());
...
int PairE3GNNParallelCPU::pack_reverse_comm_gnn(float *buf, int comm_phase) {
  std::vector<long> &idx_map = comm_index_unpack_forward[comm_phase];
  const int n = static_cast<int>(idx_map.size());
...
void PairE3GNNParallelCPU::unpack_reverse_comm_gnn(float *buf, int comm_phase) {
