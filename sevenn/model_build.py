...
def build_E3_equivariant_model(model_config: dict, parallel=False):
    ...
    use_modality = model_config[KEY.USE_MODALITY]  # boolean
    ...
    num_modalities = (
        model_config[KEY.NUM_MODALITIES]
        if use_modality and model_config[KEY.USE_MODAL_NODE_EMBEDDING]
        else 1
    )
    layers.update({
    ...
    for i in range(num_convolution_layer):
        ...
        num_modalities = (
            model_config[KEY.NUM_MODALITIES]
            if use_modality and model_config[KEY.USE_MODAL_SELF_INTER_INTRO]
            else 1
        )
        ...
        num_modalities = (
            model_config[KEY.NUM_MODALITIES]
            if use_modality and model_config[KEY.USE_MODAL_SELF_INTER_OUTRO]
            else 1
        )
    ...
    if model_config[KEY.READOUT_AS_FCN] is False:
        ...
        num_modalities = (
            model_config[KEY.NUM_MODALITIES]
            if use_modality and model_config[KEY.USE_MODAL_OUTPUT_BLOCK]
            else 1
        )
        layers.update({
