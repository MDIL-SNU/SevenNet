...
class AtomGraphDataset:
    ...
    def write_modal_attr(self, modal_type_mapper: dict, write_modal_type: bool = False):
    ...
    def get_statistics(self, key):
        ...
        def _get_statistic_dict(tensor_list):
            ...
            return {
                ...
                'max': torch.nan if data_list.numel() == 0 else float(torch.max(data_list)),
                'min': torch.nan if data_list.numel() == 0 else float(torch.min(data_list)),
            }
    ...
    def save(self, path, by_label=False):
        ...
        if by_label:
            for label, data in self.dataset.items():
                to = f'{path}/{label.strip()}.sevenn_data'
