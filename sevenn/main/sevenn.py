...
def main(args=None):
    ...
    Logger(filename=f"{os.path.abspath(working_dir)}/log.sevenn", screen=screen, rank=rank)
    Logger().greeting()

    if distributed:
        Logger().writeline(f"Distributed training enabled, total world size: {world_size}")
