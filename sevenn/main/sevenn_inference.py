...
def main(args=None):
    checkpoint, target, device, ncores, output = cmd_parse_data(args)
    ...
    inference_main(checkpoint, target, output, ncores, device)
...
def cmd_parse_data(args=None):
    ...
    ag.add_argument('-o', '--output', type=str, default=".", help="path to save inference results")
    ...
    ncores = args.ncores
    output = args.output
    return checkpoint, target, device, ncores, output
