...
def greeting():
    ...
    if not e3gnn_parallel_found:
        print("e3gnn_parallel not found, lammps parallel test will be skipped.")
...
def atoms_info_print(atoms):
    ...
    print(f"Energy per atom: {atoms.get_potential_energy(force_consistent=True) / len(atoms)}")
...
def graph_build_test():
    ...
    graph_build_cmd = f"sevenn_graph_build {OUTCAR} 4.0 -f vasp-out -o data".split()
...
def sevenn_inferece_test(cp_path: str):
    def sevenn_infer_to_atoms(inference_folder):
        ...
        if info_list is not None:
            per_graph = [{**row, KEY.INFO: info_list[i]}
        ...
        for i, row in enumerate(per_graph):
            ...
            if KEY.PRED_STRESS + "_xx" in row and row[KEY.PRED_STRESS + "_xx"] != "-":
                ...
                stress = np.array([stress_xx, stress_yy, stress_zz,
            ...
            for j, frow in enumerate(per_atom_data):
                ...
                pos.append([float(frow[KEY.POS + "_x"]),
                            float(frow[KEY.POS + "_y"]),
                            float(frow[KEY.POS + "_z"])])
                forces.append([float(frow[KEY.PRED_FORCE + "_x"]),
            ...
            atoms = ase.atoms.Atoms(numbers=species, positions=pos, cell=cell,
                                    pbc=True, info=info)
...
def sevenn_get_model_test(cp_path, is_parallel):
    ...
    if is_parallel:
        sevenn_get_model_cmd = f"sevenn_get_model -p {cp_path} -o potential/parallel".split()
    else:
        sevenn_get_model_cmd = f"sevenn_get_model {cp_path} -o potential/serial".split()
...
def sevenn_lmp_test(atoms, pair_style, pot_str,
                    test_dir, mpicmd=None, ncores=None,
                    replicate="1 1 1", rot_atoms_back=True):
    ...
    def lammps_results_to_atoms(lammps_log, force_dump):
        ...
        for i, line in enumerate(lines):
            if not line.startswith('Per MPI rank memory allocation'):
                continue
            lmp_log = {k: eval(v) for k, v in zip(lines[i+1].split(), lines[i+2].split())}
            break
        ...
        latoms.info = {'data_from': 'lammps', 'lmp_log': lmp_log, 'lmp_dump': force_dump}
...
def compare_atoms(atoms1, atoms2, rtol=1e-5, atol=1e-8):
    ...
    if not isinstance(atoms1, Atoms):
        raise TypeError(f"atoms1 should be instance of ase.Atoms, not {type(atoms1)}")
    if not isinstance(atoms2, Atoms):
        raise TypeError(f"atoms2 should be instance of ase.Atoms, not {type(atoms2)}")
    ...
    if not np.allclose(atoms1.get_cell(), atoms2.get_cell(), rtol=rtol, atol=atol):
    ...
    if not isinstance(atoms1.calc, SinglePointCalculator):
        raise TypeError(f"atoms1.calc should be instance of ase.calculator.SinglePointCalculator, not {type(atoms1.calc)}")
    if not isinstance(atoms2.calc, SinglePointCalculator):
        raise TypeError(f"atoms2.calc should be instance of ase.calculator.SinglePointCalculator, not {type(atoms2.calc)}")

    if not np.allclose(atoms1.calc.results['energy'], atoms2.calc.results['energy'], rtol=rtol, atol=atol):
    ...
    if not np.allclose(atoms1.calc.results['forces'], atoms2.calc.results['forces'], rtol=rtol, atol=atol):
        # increasing atol iteratively to find the difference
        flag = False
        for i in range(4):
            atol *= 10
            if np.allclose(atoms1.calc.results['forces'], atoms2.calc.results['forces'], rtol=rtol, atol=atol):
                flag = True
                break
        if not flag:
...
def main():
    ...
    if e3gnn_found:
        ...
        print("----------------------------------------------------")
        serial_atoms = sevenn_lmp_test(atoms, "e3gnn", seri_pot_str, "./e3gnn_test")
        print("----------------------------------------------------")
    ...
    if e3gnn_found and e3gnn_parallel_found:
        ...
        serial_333 = sevenn_lmp_test(atoms, "e3gnn", seri_pot_str, "./e3gnn_333",
                                     replicate="3 3 3")
        print("----------------------------------------------------")
        print("\nSerial 333 lammps run done")
        para_333 = sevenn_lmp_test(atoms, "e3gnn/parallel", para_pot_str,
                                   "./e3gnn_parallel_333", mpicmd="mpirun",
                                   replicate="3 3 3", ncores=2)
        print("----------------------------------------------------")
